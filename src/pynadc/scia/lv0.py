...
@dataclass()
class ConstsLV0:
    """Fixed parameters to read Sciamachy level 0 data."""

    mph_size: int = 1247
    num_aux_bcp: int = 16
    num_aux_pmtc_frame: int = 5
    num_pmd_packets: int = 200
    channel_pixels: int = 1024
...
class File:
    ...
    def __init__(self: File, flname: str, only_headers: bool = False) -> None:
        """Read whole product into memory: ascii headers and all ISPs."""
        # initialize class attributes
        self.consts = ConstsLV0()
        self.filename = flname
        self.sorted = {
            "det": True,  # assume ISP packets are sorted
            "aux": True,
            "pmd": True,
        }
        self.mph = {}
        ...
        if self.mph["TOT_SIZE"] != Path(flname).stat().st_size:
            raise SystemError(f"file {flname} incomplete")
    ...
    @staticmethod
    def __mjd_envi() -> np.dtype:
        """Return definition for a mjd record."""
        return np.dtype([("days", ">i4"), ("secnds", ">u4"), ("musec", ">u4")])

    def __fep_hdr(self: File) -> np.dtype:
        """Return definition for a front-end processor header."""
        return np.dtype(
            [
    ...
    @staticmethod
    def __packet_hdr() -> np.dtype:
        """Return definition for a packet header."""
        return np.dtype(
            [
    ...
    @staticmethod
    def __data_hdr() -> np.dtype:
        """Return definition for a data-field header."""
        return np.dtype(
            [
    ...
    def ds_hdr_dtype(self: File) -> np.dtype:
        """Return definition of common part of L0 ISP: auxiliary, detector or PMD."""
        return np.dtype(
            [
                ("mjd", self.__mjd_envi()),
    ...
    @staticmethod
    def __det_pmtc_hdr() -> np.dtype:
        """Return definition for a pmtc header."""
        return np.dtype(
            [
                ("bcps", ">u2"),
                ("pmtc_1", ">u2"),
    ...
    @staticmethod
    def __chan_hdr() -> np.dtype:
        """Return definition for a channel data header."""
        return np.dtype(
            [
                ("sync", ">u2"),
    ...
    @staticmethod
    def __clus_hdr() -> np.dtype:
        """Return definition for a pixel data block, one per cluster read-out."""
        return np.dtype(
            [
                ("sync", ">u2"),
                ("block", ">u2"),
    ...
    def __chan_data(self: File) -> np.dtype:
        """Return definition for a channel data structure."""
        return np.dtype(
            [
                ("hdr", self.__chan_hdr()),
    ...
    def det_isp_dtype(self: File) -> np.dtype:
        """Return definition for a level 0 detector ISP."""
        return np.dtype(
            [
                ("mjd", self.__mjd_envi()),
                ("fep_hdr", self.__fep_hdr()),
    ...
    def chan_dtype(self: File) -> np.dtype:
        """Return definition for science channel data."""
        return np.dtype(
            [
                ("time", "datetime64[us]"),
                ("icu_time", "u4"),
                ("temp", "f8"),
                ("coaddf", "u1", self.consts.channel_pixels),
                ("data", "f8", self.consts.channel_pixels),
            ]
        )

    # ----- auxiliary data structures -------------------------
    @staticmethod
    def __aux_pmtc_hdr() -> np.dtype:
        """Return definition for a pmtc header."""
        return np.dtype(
            [
                ("pmtc_1", ">u2"),
    ...
    def __pmtc_frame(self: File) -> np.dtype:
        """Return definition for a pmtc auxiliary frame."""
        ...
        return np.dtype(
            [
                ("bcp", aux_bcp_dtype, self.consts.num_aux_bcp),
                ("bench_rad", ">u2"),
                ("bench_elv", ">u2"),
                ("bench_az", ">u2"),
            ]
    ...
    def aux_isp_dtype(self: File) -> np.dtype:
        """Return definition for a level 0 auxiliary ISP."""
        return np.dtype(
            [
                ("mjd", self.__mjd_envi()),
                ("fep_hdr", self.__fep_hdr()),
                ...
                ("pmtc_frame", self.__pmtc_frame(), self.consts.num_aux_pmtc_frame),
            ]
        )

    # ----- PMD data structures -------------------------
    @staticmethod
    def __pmd_data() -> np.dtype:
        """Return definition for a PMD data packet."""
        return np.dtype(
            [("sync", ">u2"), ("data", ">u2", (2, 7)), ("bcps", ">u2"), ("time", ">u2")]
        )

    def pmd_isp_dtype(self: File) -> np.dtype:
        """Return definition for a level 0 auxiliary ISP."""
        return np.dtype(
            [
                ("mjd", self.__mjd_envi()),
                ...
                ("data_hdr", self.__data_hdr()),
                ("temp", ">u2"),
                ("pmd_data", self.__pmd_data(), self.consts.num_pmd_packets),
            ]
        )

    # ----- read routines -------------------------
    def __get_mph__(self: File) -> None:
        """Read Sciamachy level 0 MPH header."""

        def strip_unit(mystr: str, key: str) -> str | None:
            indx = mystr.find(key)
            if indx != -1:
                return mystr[0:indx]
            return None
    ...
    def __get_sph__(self: File) -> None:
        ...
        with open(self.filename, encoding="latin-1") as fp:
            fp.seek(self.consts.mph_size)  # skip MPH header
            ...
            for line in fp:
                ...
                if words[1][0] == '"':
                    ...
                    for unit in ["<10-6degE>", "<10-6degN>", "<deg>", "<%>", "<>"]:
                        buff = strip_unit(words[1], unit)
                        if buff is None:
                            continue
                        self.mph[words[0]] = float(buff)
    ...
    def __get_dsd__(self: File) -> None:
        ...
        with open(self.filename, encoding="latin-1") as fp:
            ...
            for line in fp:
                ...
                if words[1][0] == '"':
                    ...
                    self.dsd[num_dsd][words[0]] = int(
                        words[1][0 : words[1].find("<bytes>")]
                    )
                else:
    ...
    def bytes_remain(self: File, isp: dict, read_sofar: int = 0) -> int:
        """Return number to be read from a ISP record."""
        size = (
            isp["fep_hdr"]["length"]
            + self.__mjd_envi().itemsize
            + self.__fep_hdr().itemsize
            + self.__packet_hdr().itemsize
            + 1
        )
        size -= read_sofar
    ...
    def __get_info__(self: File) -> None:
        """Read Sciamachy level 0 DSD records."""
        ...
        with open(self.filename, "rb") as fp:
            ...
            for ni, ds_rec in enumerate(self.info):
                ...
                if num_bytes < 0:
                    print(f"# Info - read {ni} of {dsd['NUM_DSR']} ISPs")
                    break
                ...
                if ds_rec["fep_hdr"]["length"] == 6813:
                    ...
                    if flag_packet and flag_length:
                        ...
                        print(f"# Warning - ISP[{ni}] unknown packet")
        ...
        issorted = self.sorted["det"] & self.sorted["aux"] & self.sorted["pmd"]
        print(
            f"# Info - number of ISPs (sorted={issorted}):"
            f" {num_det:4d} {num_aux:3d} {num_pmd:3d}"
        )

    # repair SCIAMACHY_SOURCE_PACKETS
    def repair_info(self: File) -> None:
        ...
        for name, _id in (["det", 1], ["aux", 2], ["pmd", 3]):
            ...
            uniq, inverse, counts = np.unique(
                info["data_hdr"]["icu_time"], return_counts=True, return_inverse=True
            )
            ...
            for ii in np.where(counts == 1)[0]:
                ...
                if indx > 0 and indx + 1 < info.size:
                    if (
                        ...
                        info["data_hdr"]["icu_time"][indx] = info["data_hdr"][
                            "icu_time"
                        ][indx - 1]
                        print(f"# Info - icu_time of {name}_isp[{indx}] fixed")
                    else:
                        # print(info['data_hdr']['icu_time'][indx-1:indx+2])
                        # print(info['data_hdr']['state_id'][indx-1:indx+2])
                        # print(info['bcps'][indx-1:indx+2])
                        print(f"# Warning - (a) can not fix {name}_isp[{indx}]")
                else:
                    print(f"# Warning - (b) can not fix {name}_isp[{indx}]")
            ...
            if np.any(np.diff(info["data_hdr"]["icu_time"].astype(int)) < 0):
                ...
                for ii in range(uniq.size):
                    ...
                    blocks = np.where(np.diff(np.concatenate(([-1], indx, [-1]))) != 1)[
                        0
                    ]
                    if blocks.size == 1:
                        info_list += (info[indx],)
                    ...
                    if np.diff(blocks)[num] == 1:
                        print(f"# Warning - rejected {name}_isp: {indx}")
                        continue
    ...
    def __read_det_raw(self: File, ds_rec: np.ndarray) -> np.ndarray:
        ...
        det["pmtc_hdr"] = np.frombuffer(
            ds_rec["buff"], dtype=self.__det_pmtc_hdr(), count=1, offset=offs
        )
        ...
        for nch in range(det["pmtc_hdr"]["num_chan"]):
            channel["hdr"][nch] = np.frombuffer(
                ds_rec["buff"], dtype=self.__chan_hdr(), count=1, offset=offs
            )
            ...
            for ncl in range(channel["hdr"]["clusters"][nch]):
                hdr[ncl] = np.frombuffer(
                    ds_rec["buff"], dtype=self.__clus_hdr(), count=1, offset=offs
                )
                offs += self.__clus_hdr().itemsize

                if hdr["coaddf"][ncl] == 1:
                    nbytes = 2 * hdr["length"][ncl]
                    buff[ncl] = np.frombuffer(
                        ds_rec["buff"],
                        dtype=">u2",
                    ...
                    buff[ncl] = np.frombuffer(
                        ds_rec["buff"], dtype="u1", count=nbytes, offset=offs
                    )
                    if (nbytes % 2) == 1:
                        nbytes += 1
                offs += nbytes
    ...
    def __read_det_safe(self: File, ds_rec: np.ndarray, det_indx: int) -> np.ndarray:
        ...
        for nch in range(det["pmtc_hdr"]["num_chan"]):
            ...
            for ncl in range(channel["hdr"]["clusters"][nch]):
                ...
                if hdr["sync"][ncl] != 0xBBBB:
                    print("# Warning - cluster-sync corruption", det_indx, nch, ncl)
                    channel["hdr"]["clusters"][nch] = ncl
                ...
                if hdr["coaddf"][ncl] != 1 and 2 * hdr["length"][ncl] == bytes_left:
                    hdr["coaddf"][ncl] = 1

                if hdr["coaddf"][ncl] == 1:
                    nbytes = 2 * hdr["length"][ncl]
                    if nbytes > bytes_left:
                        print("# Warning - cluster-size corruption", det_indx, nch, ncl)
                        channel["hdr"]["clusters"][nch] = ncl
                        buff[ncl] = None
    ...
    def get_isp(self: File, state_id: list[int, ...] | None = None) -> np.ndarray:
        """Read Sciamachy level 0 ISP records into numpy compound-arrays.

        Parameters
        ----------
        state_id : list, optional
        ...
        if indx_det.size > 0:
            ...
            for ds_rec in self.info[indx_det]:
                if (
                    ...
                    and ds_rec["data_hdr"]["state_id"] not in state_id
                ):
                    continue
            ...
            print(f"# Info - read {ni} detector ISP")
        ...
        if indx_aux.size > 0:
            ...
            for ds_rec in self.info[indx_aux]:
                ...
                aux["pmtc_hdr"] = np.frombuffer(
                    ds_rec["buff"], dtype=self.__aux_pmtc_hdr(), count=1, offset=0
                )

                aux["pmtc_frame"] = np.frombuffer(
                    ds_rec["buff"],
                    dtype=self.__pmtc_frame(),
                    count=self.consts.num_aux_pmtc_frame,
                    offset=self.__aux_pmtc_hdr().itemsize,
                )
                ni += 1

            aux_isp = aux_isp[0:ni]
            print(f"# Info - read {ni} auxiliary ISP")
        ...
        if indx_pmd.size > 0:
            ...
            for ds_rec in self.info[indx_pmd]:
                ...
                pmd["temp"] = np.frombuffer(
                    ds_rec["buff"], dtype=">u2", count=1, offset=0
                )

                pmd["pmd_data"] = np.frombuffer(
                    ds_rec["buff"],
                    dtype=self.__pmd_data(),
                    count=self.consts.num_pmd_packets,
                    offset=2,
                )
                ni += 1

            pmd_isp = pmd_isp[0:ni]
            print(f"# Info - read {ni} PMD ISP")

        return (det_isp, aux_isp, pmd_isp)

    def get_channel(self: File, state_id: int, chan_id: int) -> np.ndarray:
        """Combine readouts of one science channel for a given state ID."""
            ...
            for nch in range(dsr["pmtc_hdr"]["num_chan"]):
                ...
                for ncl in range(chan_data["hdr"]["clusters"]):
                    ...
                    if chan_data["clus_hdr"]["coaddf"][ncl] == 1:
                        ...
                        buffer[:, 1:] = np.array(chan_data["clus_data"][ncl]).reshape(
                            dim, 3
                        )
